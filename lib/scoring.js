...
// Selected by the `scoring` axis ('stroke'), independent of team format.
...
export function deriveBoard(input) {
  ...
  for (const m of matches) {
    const day = dayByIndex[m.day_index] || { scoring: "match", points_per_hole: 1, play_all: true, format: m.kind };
    const scoring = day.scoring || "match";

    // Stroke-scored matches are tallied at the team/day level, not as tiles.
    if (scoring === "stroke") {
      (strokeByDay[m.day_index] ||= []).push({ m, day });
      continue;
    }

    // ---- match-play path (any team format) — UNCHANGED scoring ----
  ...
  for (const dayIndex of Object.keys(strokeByDay).map(Number).sort((a, b) => a - b)) {
    ...
    strokeDays.push({
      dayIndex,
      scoring: "stroke",
      format: (dayByIndex[dayIndex] || {}).format || "scramble",
      teamATotal,
      teamBTotal,
      aThru,
      bThru,
  ...
  // Live feed: most-recent newsworthy events, newest first, with a side tag.
  const ticker = (events || []).slice(0, 8).map((e, i) => ({
    id: e.created_at ? `${e.created_at}-${i}` : `${i}-${e.text}`,
    text: e.text,
    hole: e.hole,
    side: e.dot === t.team_a_color ? "A" : e.dot === t.team_b_color ? "B" : null,
  }));
